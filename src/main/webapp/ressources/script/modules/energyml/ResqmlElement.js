...
import {
    ...
    createHoverableHtmlContent,
    createInputGroup,
    geosiris_createEditableSelector,
    getPopoverOpenUUID
} from "../UI/htmlUtils.js";
import {
    openResqmlObjectContentByUUID,
    saveResqmlObjectByUUID
} from "../main.js";
import {
    generateUUID,
    isResqmlListType,
...
import {
    extTypeAttributes,
    mapResqmlEnumToValues,
    mapResqmlTypeToSubtypes,
    mapResqmlTypesComment,
    openHDFViewAskH5Location
} from "./epcContentManager.js";
import {
    closeModal,
    openModal
} from "../UI/modals/modalEntityManager.js";
import {
    createTableFilterInput,
    createTableFromData,
    transformTabToFormCheckable,
    transformTabToFormRadio
} from "../UI/table.js";
import {
    UUID_REGEX,
    UUID_REGEX_str,
            ...
            if (this.type.includes("CustomData")) {
                                ...
                                if (subA.isResqmlListType()) {
                                    ...
                                    if (this.isResqmlListType()) {
                                        ...
                                        if (!subAElt.lastChild.previousSibling.className.includes("draggableButton") && this.attributes.length > 1) {
                                            ...
                                            geosiris_DndHandler.applyDragEvents(constButDrag, function() {
                                                updateListEltIdx(currentDraggableElt_html, currentDraggableElt);
                                            });
                ...
                if (typeInTitle.length > 0){
                    ...
                    if (mapResqmlTypesComment[this.type.toLowerCase()] != null) {
                        ...
                        htmlTitleElt_type.appendChild(commentType);
                        var divCommentTitle = createHoverableHtmlContent(commentType, '<span class="treeLeafWithProperty">' + this.type + "</span> : " + mapResqmlTypesComment[this.type.toLowerCase()], false);

                        this.htmlTitleElt.appendChild(divCommentTitle);
                    } else {
                        //console.log("no comment for " + this.type);
                    }
                    htmlTitleElt_type.appendChild(document.createTextNode("]"));
                }
                        ...
                        this.htmlTitleElt.onclick = function() {
                            for (var propChildIdx = 0; propChildIdx < propRootElt.childNodes.length; propChildIdx++) {
                                try {
                                    ...
                                    console.error("/!\\ child not disable : " + propChildIdx);
                                }
                            }
                ...
                getJsonObjectFromServer("ResqmlAccessibleDOR?uuid=" + constThis.rootUUID +
                    ...
                    function(jsonContent) {
                        ...
                        var tableDOR = createTableFromData(
                            ...
                            ["Num", "Type", "Title", "UUID", "SchemaVersion"],
                            workspace_content.map(contentElt => [function() {
                                openResqmlObjectContentByUUID(contentElt['uuid'])
                            }]),
                            null);
